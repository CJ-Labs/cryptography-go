...
// 优化 ellipticCurveMultiply 函数，使用更高效的算法
...
func generateSignature(privateKey *big.Int, message []byte) (*big.Int, *big.Int, error) {
	// 使用 SHA-256 哈希函数对输入的消息进行哈希处理，生成一个固定长度的哈希值（32 字节）。
	messageHash := sha256.Sum256(message) // 对消息进行哈希处理
	...
	for {
		// 生成随机数 k
		// 在循环中生成一个随机数 k，范围在曲线的顺序 curveOrder 之内。
		randomK, err = rand.Int(rand.Reader, curveOrder)
		if err != nil {
			return nil, nil, err
		}

		// 生成的 (xCoordinate,yCoordinate) 是基点 G 经过 k 步椭圆曲线乘法得到的点
		xCoordinate, yCoordinate := ellipticCurveMultiply(Gx, Gy, randomK) // 计算椭圆曲线点
		...
		r = new(big.Int).Mod(xCoordinate, curveOrder) // r = x mod n
...
func Test_generate_ecdsa(t *testing.T) {
	...
	pubKeyX, pubKeyY := calculatePublicKey(privKey) // 计算公钥
